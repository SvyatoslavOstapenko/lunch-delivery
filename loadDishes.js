...
window.loadDishes = loadDishesFromAPI;// loadDishes.js - ОБНОВЛЕННАЯ ВЕРСИЯ
...
async function loadDishesFromAPI() {
    try {
        ...
        const transformedData = data.map(item => {
            ...
            // Добавляем ID блюда (из API)
            const dishId = item.id || item.keyword;
            
            return {
                id: dishId, // Добавляем ID для отправки на сервер
                keyword: item.keyword,
                name: item.name,
                price: item.price,
                category: category,
...
// Экспортируем функцию
window.loadDishes = loadDishesFromAPI;// loadDishes.js - Убедитесь, что функция возвращает блюда с ID
...
async function loadDishesFromAPI() {
    try {
        ...
        const transformedData = data.map(item => {
            ...
            // ВАЖНО: Сохраняем ID блюда
            const dishId = item.id; // Используем числовой ID из API
            
            return {
                id: dishId, // Числовой ID для отправки на сервер
                keyword: item.keyword,
                name: item.name,
                price: item.price,
                category: category,
